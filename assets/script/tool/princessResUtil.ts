...
import princessResBind from "./princessResBind";
import { princessControl } from "./princessControl";

export type AssetCallFunc = (err: Error, assetData: cc.Asset) => void;
 
export class princessResUtil {
     
    // 绑定资源释放
    public static bindResAsset(parent: cc.Node, asset: cc.Asset): void {
        let tempAuto = parent.getComponent(princessResBind);
        if (!cc.isValid(tempAuto)) {
            tempAuto = parent.addComponent(princessResBind);
        }
